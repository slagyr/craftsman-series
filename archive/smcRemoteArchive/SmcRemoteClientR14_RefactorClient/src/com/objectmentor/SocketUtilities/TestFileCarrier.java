...
public class TestFileCarrier extends TestCase {
  ...
  public void testFileCarrier() throws Exception {
    ...
    FileCarrier fc = new FileCarrier(null, "testFileCarrier.txt");
    ...
    assert("files aren't the same.", filesAreTheSame(newFile, sourceFile));

    assert("newfile", newFile.delete());
    assert("oldFile", sourceFile.delete());
    assert("directory", tmpDirectory.delete());
  }
